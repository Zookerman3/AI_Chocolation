...
describe('toCSV', () => {
  it('emits a header row even with no records', () => {
    expect(toCSV([])).toBe(
      'box_id,box_size,method,demo,location_id,started_at,completed_at,duration_ms,undo_count,flavor_id,flavor_name,piece_count\n',
    )
  })
  ...
  it('writes the location on every row of a box that has one, and leaves it empty otherwise', () => {
    const lines = toCSV([{ ...record, id: 'r2', locationId: 'bradley' }, record]).trim().split('\n')
    expect(lines[1]).toBe('r2,6,tap,false,bradley,2026-09-14T10:00:00.000Z,2026-09-14T10:00:07.000Z,7000,1,amaretto,Amaretto,4')
    expect(lines[2]).toContain(',bradley,')
    expect(lines[3]).toBe('r1,6,tap,false,,2026-09-14T10:00:00.000Z,2026-09-14T10:00:07.000Z,7000,1,amaretto,Amaretto,4')
  })
})
